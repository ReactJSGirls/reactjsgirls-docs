...
We have the all the URL's of the cats in the array and we know that we want to remove the one that is attached to the that `li` so what we want to do is leave all the cats in the array except the one where the url matches the one of the cat we clicked.
